...
public class Scrabble {
    ...
    private static void playWord(int x, int y, int contact_point, Frame frame, boolean direction, Tile[] letters_on_board, String word, tileType[] bonus_squares){
        ...
        int count = 0;
        for(int i = 0; i < letters_on_board.length; i++){
    ...
    //This creates an array with the squares that the tiles will be placed on e.g. double word
    private static tileType[] populateArrayWithBoardSquares(int x, int y, boolean direction, int wordLength, Tile[] squares_without_tiles){
        ...
        if(checkSpace(x, y, direction, wordLength)) {
            if (direction) {
                for (int i = 0; i < wordLength; i++) {
                    //doesn't consider squares with tiles on them
    ...
    private static void exchangeLetters(String command, String letters, Frame frame){
        ...
        LinkedList<Tile> replace_these_letters = new LinkedList<Tile>();
        boolean valid=true;
        if(command.equals("EXCHANGE")){
            for(int i = 0; i < letterArray.length; i++) {
                //checks the players frame for the tile they want to play
                if(!frame.checkForTile(frame.getTile(letterArray[i]))){
                    valid=false;
                } else{
                    replace_these_letters.add(frame.getTile(letterArray[i]));
                    frame.removeLetters(letterArray[i]);
                }
            }
        }
        if(valid){
    ...
    private static int score(String word, tileType[] board_squares, int bonus){
        ...
        for(int i = 0; i < letters.length; i++){
            if(board_squares[i] == tileType.DOUBLE_LETTER){
                ...
                //keep track of number of double word and triple word tiles
                score = score + Pool.query(letters[i]);
                num_double_words++;
        ...
        //multiply score by double word and triple word at end
        ...
        //player gets a bonus 50 points for using all 7 tiles
        if(bonus == 7){
            score = score + 50;
        }
